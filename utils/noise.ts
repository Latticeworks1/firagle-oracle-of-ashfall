...
class ImprovedNoise {
    ...
    constructor() {
        ...
        102,143,58, 4,167,54,65,25,63,161, 1,216,80,73,209,76,132,187,208, 89,18,169,200,196,
...
export const generateHeightData = (): Float32Array => {
    ...
    for (let j = 0; j < TERRAIN_HEIGHT; j++) {
        for (let i = 0; i < TERRAIN_WIDTH; i++) {
            let quality = 1, totalHeight = 0;
            for (let k = 0; k < ITERATIONS; k++) {
                totalHeight += Math.abs(perlin.noise((i / quality) * FREQUENCY, (j / quality) * FREQUENCY, z)) * quality;
                quality *= 4.5;
            }
            data[i + j * TERRAIN_WIDTH] = totalHeight;
