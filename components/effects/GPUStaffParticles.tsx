...
export default forwardRef<EmitterAPI, Props>(function GPUStaffParticles(
  ...
  const { geo, mat } = useMemo(() => {
    ...
    const vsh = /* glsl */`
      ...
      void main(){
        ...
        // Size fade with variation
        float sizeBase = mix(uEndSize, uStartSize, 1.0 - smoothstep(0.0, 1.0, vLifeT));
        float sizeVariation = 1.0 + uSizeVariation * sin(iSeed * 6.2831 + uTime * 2.0); // Per-particle size pulse
        float size = sizeBase * sizeVariation;
    ...
    const fsh = /* glsl */`
      ...
      uniform vec3 uColors[4]; // Array of colors
      ...
      void main(){
        ...
        // Cycle through colors based on seed and time
        float colorIndex = mod(floor(vLifeT * uColorCount + vGlowMix * 2.0), uColorCount);
        vec3 baseColor = uColors[int(colorIndex)];
        vec3 finalColor = mix(uColorA, mix(uColorB, baseColor, vGlowMix), vGlowMix);
    ...
    const mat = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uColors:     { value: [
          new THREE.Vector3(1.0, 0.5, 0.2), // Warm Gold
          new THREE.Vector3(0.2, 0.7, 1.0), // Cool Blue
          new THREE.Vector3(0.8, 0.2, 0.8), // Purple
          new THREE.Vector3(0.9, 0.9, 0.2)  // Pale Yellow
        ] },
        uColorCount: { value: 4 },
  ...
  useImperativeHandle(ref, () => ({
    emit(count = BURST_SIZE, pos, dir, speed = 6) {
      ...
      for (let n = 0; n < count; n++) {
        ...
        const axis = new THREE.Vector3().crossVectors(forward, new THREE.Vector3(20, 40, 20));
        ...
        const spd = speed * (0.6 + Math.random() * 2.8);
  ...
  useFrame((state, delta) => {
    ...
    mat.uniforms.uSizeVariation.value = Math.sin(clock.current * 0.5) * 0.8; // Dynamic size variation
